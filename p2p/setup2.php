...
$htop = $htop .'    <!-- Le styles -->';
$htop = $htop .'    <link href="http://www.glimworm.com/_assets/moock/bootstrap204/css/bootstrap.css" rel="stylesheet">';
$htop = $htop .'	<link rel="stylesheet" type="text/css" href="lib/css/prettify.css">';
$htop = $htop .'    <link href="http://www.glimworm.com/_assets/moock/bootstrap204/css/bootstrap-responsive.css" rel="stylesheet">';
...
$htop = $htop .'<link href="setup.css" rel="stylesheet">';
...
$hbot = $hbot .'    <script src="http://www.glimworm.com/_assets/moock/bootstrap204/js/bootstrap.min.js"></script>';
$hbot = $hbot .'    <script src="http://www.glimworm.com/_assets/moock/bootstrap204/js/spin.js"></script>';
/*
...
//	message
  ...
  PRIMARY KEY (`id`)
	...
	$dbhost = ($_POST["dbhost"] != "") ? $_POST["dbhost"] : $dbhost;
		...
		mysql_query("delete from domain where Pagename='setup'");
		...
		if (getConfiguration("setup",null) == null) {
			...
			$contentbits = explode("\n",$content_load);
			$content = "";
			foreach ($contentbits as $line) {
				if (strpos($line, "wiz:") !== 0) {
...
|-->";
			...
			mysql_query("insert ignore into pages (ID,Pagename,Caption,img,bodytext,CatID,template,Volgorde,type,extraData) values (0,'Twitter','Twitter','','".esc($content)."',1,'',1,'TI','{ \"navBar\" : \"y\" }')");
...
$url = str_replace("setup2.php", "", $url);
...
//	echo "<tr><td width='40%'>Site Title</td><td width='60%'><input name='sitename' value='$sitename'></td></tr>";
	...
	if ($hosted == "y") {
		echo "<h2>You're almost there! we just need some additional details</h2>";
	} else {
	...
	if (!$hosted || $hosted != 'y') {
		...
		echo "<div id='step2-adminemail' class='control-group rel'>
				<label class='control-label'>E-Mail address</label>
				<span class='spinner'></span>
				<input class='input-block-level' name='adminemail' placeholder='We will send the link to your cms to this address' type='text' value='$adminemail'/>
				<div class='help-block'></div>
			</div>";
			
		echo "<div id='step2-password' class='control-group'>
		...
		echo "	<h2>Fill in at least one of the fields below so we can already put content in your app!</h2>
					...
					<div id='step2-facebook' class='control-group'>
						...
						<div class='input-prepend input-append'>
							...
							<input id='facebook' class='span5' placeholder='Enter your Facebook name' type='text' />
							<span clas='add-on'>
								<button id='facebook-oauth' class='btn disabled' type='button'>Connect</button>
							</spen>
							<div class='help-block'></div>
						</div>
					</div>
					
					<div id='step2-linkedin' class='control-group'>
						<label class='control-label'>LinkedIn</label>
						<div class='input-prepend input append'>
							<span class='add-on'>linkedin.com/</span>
							<input id='linkedin' class='span5' placeholder='Enter your LinkedIn ID' type='text' />
							<span clas='add-on'>
								<button id='linkedin-oauth' class='btn disabled' type='button'>Connect</button>
							</spen>
							<div class='help-block'></div>
						</div>
					</div>
...
//		echo "<tr><td>Admin Password</td><td><input name='MASTER_PASSWORD' value='$MASTER_PASSWORD'></td></tr>";
		...
		/*
		require_once("./plugins/connectors/linkedin/connect.php");
		$linkedin = new linkedin();
		$linkedin->etc(dirname(__FILE__)."/etc/");
		
		if ($linkedin->check() == true) {
			$user = $linkedin->fetch('GET', '/v1/people/~:(firstName,lastName)');

			
			echo "<h1>USER : $user->firstName $user->lastName</h1>";
			print "<pre>";
			print "Access token " . $linkedin->access_token;
			print "</pre>";
			print "<pre>";
			var_dump($user);
			print "</pre>";
		} else {
			print "<h1>Linkedin not connected</h1>";
			print "<pre>";
			$linkedin->load();
			var_dump($linkedin);
			print "</pre>";
