...
if ( !class_exists('wikidata') ) {
	...
	class wikidata {
		...
		var $rest_endpoint = "http://www.wikidata.org";		
		...
		function wikidata ($api_key, $secret = NULL, $die_on_error = false) {
...
//			var_dump($this->parsed_response["meta"]);
		...
		function getpage ($q = "") {
			$this->request();
//			$this->request("/wiki/".$q, array());
			$this->request("/w/api.php?".$q, array(
